  ...
  if (!$stmt) {
    ...
    if ($res->num_rows === 1) {
      ...
      elseif (!password_verify($password, $u['password_hash'])) {
        ...
        // Reset old queue settings for a fresh login
        unset($_SESSION['counter_id'], $_SESSION['selected_queue_group'], $_SESSION['active_counters']);
  ...
  <main class="relative w-full max-w-md">
    <div class="bg-white/80 backdrop-blur rounded-3xl border border-white/50 shadow-2xl p-7 sm:p-8">
      ...
      <p class="mt-5 text-xs text-slate-500">
        Tip: counter availability is controlled inside Staff Queue Control using the Active/Inactive checkboxes.
      </p>
    </div>
  </main>
</body>
